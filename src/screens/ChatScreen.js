...
import {
  ...
  Linking, ActionSheetIOS, Clipboard, Keyboard,
} from 'react-native';
...
export default function ChatScreen({ route, navigation }) {
  ...
  // Keyboard detection for responsive padding
  useEffect(() => {
  ...
  const startRecording = async () => {
    // Limpiar si por alguna razón ya había una grabación activa trabada
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      Alert.alert('Error al enviar', 'Detalle: ' + err.message);
    } finally {
      setSending(false);
    }
  };
